...
function getRankTitle(level: number, isEnglish: boolean): { title: string; icon: string } {
  if (level >= 34) return { title: isEnglish ? 'Grand Explorer' : 'Penjelajah Agung', icon: '👑' };
  if (level >= 26) return { title: isEnglish ? 'Legendary Adventurer' : 'Petualang Legendaris', icon: '🌟' };
  if (level >= 18) return { title: isEnglish ? 'Expert Explorer' : 'Penjelajah Ulung', icon: '⚔️' };
  if (level >= 10) return { title: isEnglish ? 'Intermediate Adventurer' : 'Petualang Madya', icon: '🗡️' };
  if (level >= 5)  return { title: isEnglish ? 'Young Explorer' : 'Penjelajah Muda', icon: '🧭' };
  return { title: isEnglish ? 'Novice Adventurer' : 'Petualang Pemula', icon: '🌱' };
}
...
export default function ProfilePage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setSaveError(e?.response?.data?.error ?? (isEnglish ? 'Failed to save profile.' : 'Gagal menyimpan profil.'));
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const handleAvatarUpload = async (e: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      alert(e?.response?.data?.error ?? (isEnglish ? 'Failed to upload avatar.' : 'Gagal upload avatar.'));
    }
  };

  const name               = user?.username ?? (isEnglish ? 'Explorer' : 'Petualang');
  ...
  const rank               = getRankTitle(level, isEnglish);
      ...
      <div className="relative z-10 max-w-4xl mx-auto px-4 md:px-6 py-6 pb-20 space-y-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-black leading-none" style={{ color: '#1a0f0a' }}>
                ...
                <span style={{ color: '#F14C38' }}>{t?.profile?.title?.split(' ')[1] || 'PROFILE'}</span>
              </h1>
        ...
        >
          <div className="relative px-6 pt-5 pb-16"
            style={{ background: 'linear-gradient(135deg, #F14C38 0%, #c93020 100%)', borderBottom: '4px solid #1a0f0a' }}>
            ...
            <div className="relative z-10 flex items-start justify-between">
              <div>
                ...
                <h2 className="text-3xl font-black text-white leading-none uppercase">{name}</h2>
              ...
              {streakDays > 0 && (
                <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl"
                  ...
                  <span className="text-sm font-black text-white">
                    {streakDays} {isEnglish ? 'days' : 'hari'}
                  </span>
                </div>
              )}
            </div>
              ...
              <div className="flex-1 pt-12">
                <div className="flex items-center justify-between mb-1.5">
                  <span className="text-[10px] font-black uppercase tracking-widest" style={{ color: 'rgba(26,15,10,0.45)' }}>
                    {isEnglish ? `EXP to Level ${level + 1}` : `EXP ke Level ${level + 1}`}
                  </span>
                  <span className="text-[10px] font-black" style={{ color: '#F14C38' }}>
                    {xpToNext > 0
                      ? (isEnglish ? `${xpToNext} XP more` : `${xpToNext} XP lagi`)
                      : 'MAX!'}
                  </span>
                </div>
                ...
                <div className="text-right mt-1">
                  <span className="text-[9px] font-black uppercase" style={{ color: 'rgba(26,15,10,0.35)' }}>
                    {xp} {isEnglish ? 'Total XP' : 'XP Total'}
                  </span>
                </div>
              </div>
            </div>
              ...
              <div className="p-6 space-y-5">
                <div className="flex items-center gap-4">
                  ...
                  <p className="text-xs font-bold" style={{ color: 'rgba(26,15,10,0.5)' }}>
                    {isEnglish ? 'Click photo to upload new avatar' : 'Klik foto untuk upload avatar baru'}
                  </p>
                </div>
                ...
                <div>
                  ...
                  <div className="flex gap-3">
                    {(['male', 'female'] as const).map((g) => (
                      ...
                      >
                        {g === 'male'
                          ? (isEnglish ? '👦 Male' : '👦 Laki-laki')
                          : (isEnglish ? '👧 Female' : '👧 Perempuan')}
                      </button>
                    ))}
                  </div>
                </div>
                ...
                <div className="flex gap-3 pt-1">
                  ...
                  <button
                    ...
                    {...pressHandlers}
                  >
                    {isSaving ? <Loader2 size={15} className="animate-spin" strokeWidth={3} /> : <Check size={15} strokeWidth={3} />}
                    {isSaving
          ...
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              ...
              <p className="text-[9px] font-black uppercase tracking-widest leading-none mt-0.5" style={{ color: '#F14C38' }}>
                {badgeCount} {isEnglish ? 'badges earned' : 'badge diraih'}
              </p>
            </div>
          </div>
            ...
            >
              <div className="px-6 py-4 flex items-center justify-between"
                ...
                <span className="font-black uppercase tracking-widest text-sm text-white">
                  {isEnglish ? 'Confirm Logout' : 'Konfirmasi Keluar'}
                </span>
                <button
                  onClick={() => setShowLogoutConfirm(false)}
                  className="w-8 h-8 flex items-center justify-center rounded-xl"
              ...
              <div className="p-6 text-center space-y-5">
                ...
                <div>
                  <p className="font-black text-lg uppercase" style={{ color: '#1a0f0a' }}>
                    {isEnglish ? 'Are you sure you want to logout?' : 'Yakin ingin keluar?'}
                  </p>
                ...
                <div className="flex gap-3">
                  <button
                    ...
                    style={{
                      ...
                      border: '3px solid rgba(26,15,10,0.2)', transition: 'all 0.1s'
                    }}
                  >
                    {isEnglish ? 'Cancel' : 'Batal'}
                  </button>
                  <button
                    ...
                    style={{
                      ...
                      border: '3px solid #1a0f0a', boxShadow: '3px 3px 0 #1a0f0a', transition: 'all 0.1s'
                    }}
                    {...pressHandlers}
                  >
                    <LogOut size={14} strokeWidth={3} /> {t?.profile?.logout}
